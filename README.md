...
> **解释器 = `~/miniconda3`**：与工作区通则一致（`AGENTS.md`「运行环境」：Python 一律 `~/miniconda3`；本机具体版本以现场 `~/miniconda3/bin/python3 -V` 读数为准，不在本文档写死）。**依赖清单（5 枚，装在 conda，不用 `--user`；包名形态与 `bootstrap/SETUP.md` §3.1「依赖包」表的 web 行逐项一致）**：`WSGIserver`（提供 `wsgiserver`）、`wsgi-basic-auth`（提供 `wsgi_basic_auth`，Requires `webob`）、`chardet`（提供 `chardet`）、`WebOb`（提供 `webob`）、`legacy-cgi`（提供 `cgi`，**分档依赖**，见下）。**`legacy-cgi` 是可判形态的分档，不是无条件必需**：按本机 `~/miniconda3/bin/python3 -V` 判——`<3.13` 由 stdlib 提供 `cgi`、不需装；`>=3.13`（`cgi` 已从标准库移除）必须装 `legacy-cgi`，因为 `core/handler.py:parse_post` 用 `cgi`（`cgi.parse_header` / `cgi.parse_multipart` 解 multipart 上传），且 `webob/compat.py` 有**模块级** `from cgi import parse_header` ⇒ 在 `>=3.13` 的机器上 `legacy-cgi` 必须与 `WebOb` 同装（缺它连 `import webob` 都炸）。**`chardet` 缺失的失败形态是静默的**：全仓唯一 import = `stores/dir_store.py:11`（`:13 chardet.detect(content)['encoding']`），缺包时 Dir/Enc store 的挂载异常被 `stores/store.py` 的 `except` 吞成 `mount skipped` WARNING ⇒ 8080 仍返 **401**、日志**零 Traceback**、而**所有内容路径 404**（看着像服务正常、实则内容面全灭；dev 侧实测吃过一次 2 分 54 秒降级窗，原文 = `agents/task/jckmms/report.md` §7.2）。**重启前自检（5 模块）**：`~/miniconda3/bin/python3 -c 'import cgi, wsgiserver, wsgi_basic_auth, webob, chardet'`（无输出 = OK）。**服务定义形态**：`env/services.yml` 的 web `cmd: ["~/miniconda3/bin/python3", w/core/server.py, 0.0.0.0:8080, run/logs/web.log]` 走绝对解释器路径（`svc/svc.py:_expand` 对 cmd 逐项 `os.path.expanduser` ⇒ `~` 可用、各机 home 路径不同也可用）；`match: ['^\S*python3? w/core/server\.py 0\.0\.0\.0:8080']` 仍锚定行首、但放行解释器路径前缀：web 的 online 判定走 `status: {http: …}`（不经 match），而 `svc.py stop web` 的杀进程目标集 = `match` + `stop_match` 对真实 cmdline 的 `re.search` 命中 ⇒ 换解释器后不同批改 match 会 **stop 不掉旧进程**（而 status 仍报 online，零信号）。
