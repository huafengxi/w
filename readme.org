...
calls =register(REGISTRY)= in each =ext/<feature>/register.py=; store classes,
mime rules, vmap/fstab fragments, =PATH= bin dirs, and startup hooks all
register through this harness. Default =ext= is the full ordered list
=introspect,dsync,org,markdown,encrypt,shell,sql,webdav,fileops,media= (media
last — it owns the =timestamp-server.py= startup hook). Set =ext= empty for a
...
+ shared playback state is broadcast on TCP port 23554 via =timestamp-server.py=
  (auto-launched by the =ext/media= startup hook); media views post state through =view/state_reporter.js=.
...
+ mount a WebDAV endpoint via =fstab=, e.g. =/dav WebDav https://host user pass /=,
  see =ext/webdav/webdav_store.py= (requires =pip install webdav4=; optional =httpx[socks]= if the endpoint needs a SOCKS proxy).
