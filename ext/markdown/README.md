...
+ 链接带 `_v=inline`：点击不跳转，改为在链接下方就地展开一个 iframe 预览（目标页就地嵌入，高度随内容自适应），再点收起；可同时展开多个；ctrl/meta/shift+点击或中键点击不拦截（走浏览器默认新 tab 行为）。`_v=` 是客户端私有参数命名空间（下划线前缀约定）：服务端不消费，构造内嵌 iframe 的 src 前剥离，其余参数保留。
+ 本视图被嵌进外层容器（iframe 内）时，顶栏自动隐藏。

## 参数

+ 本视图无自身消费的 URL 参数。
+ 周期性刷新由容器层承担（frame 容器行级 `refresh=N` 属性，见 `../frame/`），
  本视图不处理。
...
# 本工作区典型用法：report 树 ido 派发——缺省端点把 `${<sub> [args...]}`
# 派发到由文档路径推导出的 report 树 ido 的执行项 <sub>，结果以 markdown
# 流式内嵌（派发约定见 ../shell/ 的 sh.rc）：
${summary}
```
