...
7. SqlStore — 用关系数据库实现的store (见 =sql_store.py=).

每种store对应 =w/<type>_store.py=，由 =store.get_store_cls= 按 =fstab= 中的类型按需加载。

** playback state broadcast
=web.py= 启动时会同时拉起 =timestamp-server.py= 监听TCP 23554，
