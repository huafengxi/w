...
URL query string 里的 =v= 优先级更高，例如 [[?v=read]]。POST body 会被当作
query string 一起并入 args。

** view_map
=mime type= 到 =view_path= 的映射叫 view_map，服务时通过 [[/vmap/]] 暴露。
...
3. PyEnvStore — 把当前 python 进程的全局变量暴露为 store，用于 introspect ([[/g/]])。
